...
/**
	Constructor
*/
Segment::Segment(const Point3d& a_, const Point3d& b_){
	this->a = a_;
	this->b = b_;
}
...
void Segment::getClosestPointTo(const Point3d& c, Point3d* result) const{
	...
	*result = a + v*mu; //->setToOffsetFromPoint(a, v, mu);
}
...
void Segment::getShortestSegmentTo(const Segment& other, Segment* result) const{
	...
	if (IS_ZERO(C*D - B*B)){
		...
		result->a = this->a + tmp2*((mu_a2 + mu_b2)/2.); //.setToOffsetFromPoint(this->a, tmp2, (mu_a2 + mu_b2)/2.0);
	...
	result->a = this->a + tmp2*mu_c; //.setToOffsetFromPoint(this->a, tmp2, mu_c);
	result->b = other.a + tmp3*mu_d; //.setToOffsetFromPoint(other.a, tmp3, mu_d);
}
