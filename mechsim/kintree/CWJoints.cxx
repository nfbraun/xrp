...
{
    return S_x(-par[pid(1)]) * S_y(-par[pid(0)]);
}
...
{
    return S_x(-par[pid(2)]) * S_y(-par[pid(1)]) * S_z(-par[pid(0)]);
}
...
{
    switch(i) {
        case 0: return -S_x(-par[pid(1)]) * dS_y_dp(-par[pid(0)]);
        case 1: return -dS_x_dp(-par[pid(1)]) * S_y(-par[pid(0)]);
        default: throw std::runtime_error("ParameterID out of range");
    }
}
...
{
    switch(i) {
        case 0: return -S_x(-par[pid(2)]) * S_y(-par[pid(1)]) * dS_z_dp(-par[pid(0)]);
        case 1: return -S_x(-par[pid(2)]) * dS_y_dp(-par[pid(1)]) * S_z(-par[pid(0)]);
        case 2: return -dS_x_dp(-par[pid(2)]) * S_y(-par[pid(1)]) * S_z(-par[pid(0)]);
        default: throw std::runtime_error("ParameterID out of range");
    }
}
...
Eigen::Matrix4d InvAnkleJoint::d2S_dij(const Eigen::VectorXd& par, unsigned int i, unsigned int j) const
{
    if(i > 1 || j > 1) throw std::runtime_error("ParameterID out of range");
    switch(2*i+j) {
        case 0:         return S_x(-par[pid(1)]) * d2S_y_dpp(-par[pid(0)]);
        case 1: case 2: return dS_x_dp(-par[pid(1)]) * dS_y_dp(-par[pid(0)]);
        case 3:         return d2S_x_dpp(-par[pid(1)]) * S_y(-par[pid(0)]);
    }
}
...
Eigen::Matrix4d InvHipJoint::d2S_dij(const Eigen::VectorXd& par, unsigned int i, unsigned int j) const
{
    if(i > 2 || j > 2) throw std::runtime_error("ParameterID out of range");
    switch(3*i+j) {
        case 0:         return S_x(-par[pid(2)]) * S_y(-par[pid(1)]) * d2S_z_dpp(-par[pid(0)]);
        case 1: case 3: return S_x(-par[pid(2)]) * dS_y_dp(-par[pid(1)]) * dS_z_dp(-par[pid(0)]);
        case 4:         return S_x(-par[pid(2)]) * d2S_y_dpp(-par[pid(1)]) * S_z(-par[pid(0)]);
        case 5: case 7: return dS_x_dp(-par[pid(2)]) * dS_y_dp(-par[pid(1)]) * S_z(-par[pid(0)]);
        case 6: case 2: return dS_x_dp(-par[pid(2)]) * S_y(-par[pid(1)]) * dS_z_dp(-par[pid(0)]);
        case 8:         return d2S_x_dpp(-par[pid(2)]) * S_y(-par[pid(1)]) * S_z(-par[pid(0)]);
    }
}
